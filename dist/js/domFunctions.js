...
export const displayError = (headerMsg, srMsg) => {
  ...
  const updateWeatherLocationHeader = (message) => {
    let h1 = document.getElementById("#currentForecast__location");
...
const getWeatherClass = (icon) => {
    ...
    const lastChar = icon.s;isContext(2);
    const weatherLookup = {
        "09": "snow",
        10: "rain",
        11: "rain",
...
const buildScreenReaderWeather = (weatherJson, locationObj) => {
    ...
    const tempUnit = unit === "imperial" ? "F" : "C"; //if unit equals imperial, make tenary statement: it will equal Farenheit, otherwise it will equal Celcius
...
const createCurrentConditionsDivs = (weatherObj, unit) => {
    ...
    const feels = createElem("div", "feels" `Feels like ${Math.round(Number(weatherObj.current.feels_like))}°`); //alt 0176
...
const getDayAbbreviation = (data) => {
    const dateObj = new Date(data * 1000); //data we're getting from api
